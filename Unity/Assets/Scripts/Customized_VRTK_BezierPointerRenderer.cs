...
/* XXX a complete copy-and-change of the class VRTK_BezierPointerRenderer from VRTK.
 * See the new functionality in the last method. 
 * 
 * Indentation in this file is kept for reasons of minimizing changes to the original.
 */
    ...
    /// </example>
    public class Customized_VRTK_BezierPointerRenderer : VRTK_BasePointerRenderer
    {
        ...
        protected virtual Vector3 ProjectForwardBeam()
        {
            ...
            var hasRayHit = IssueRaycast(pointerRaycast, out collidedWith, calculatedLength, ~layersToIgnore);
        ...
        protected virtual Vector3 ProjectDownBeam(Vector3 jointPosition)
        {
            ...
            var downRayHit = IssueRaycast(projectedBeamDownRaycast, out collidedWith, float.PositiveInfinity, ~layersToIgnore);
        ...
        protected virtual void AdjustForEarlyCollisions(Vector3 jointPosition, Vector3 downPosition)
        {
            ...
            if (collisionCheckFrequency > 0)
            {
                ...
                for (int i = 0; i < tracerDensity - checkFrequency; i += checkFrequency)
                {
                    ...
                    if (IssueRaycast(checkCollisionRay, out checkCollisionHit, nextPointDistance, ~layersToIgnore))
                    {
                        ...
                        if (IssueRaycast(downwardCheckRay, out downwardCheckHit, float.PositiveInfinity, ~layersToIgnore))
                        {
        ...
        /* XXX -- AR -- XXX
         * 
         * Hacked in the script because we can't do it otherwise
         */
        public float physicsSphereRadius = 0;

        protected bool IssueRaycast(Ray ray, out RaycastHit hitinfo, float maxDistance, int layerMask)
        {
            if (physicsSphereRadius <= 0)
                return Physics.Raycast(ray, out hitinfo, maxDistance, layerMask);
            else
            {
                if (Physics.OverlapSphere(ray.origin, physicsSphereRadius, layerMask).Length > 0)
                {
                    hitinfo = new RaycastHit();
                    return false;
                }
                return Physics.SphereCast(ray, physicsSphereRadius, out hitinfo, maxDistance, layerMask);
            }
        }
    }